...
# ==========================================
    ...
    with st.sidebar:
        st.header("⏸ Menu")
        st.info("💡 **Auto-Save Active:** Your progress is saved every time you click. Copy the code below before leaving!")
        current_save = generate_save_code()
        st.text_input("Latest Save Code (Copy this):", value=current_save, disabled=False)
    ...
    if not st.session_state.simulation_mode:
        if not is_locked:
            ...
            q_encoded = urllib.parse.quote(q['question_text'])
            st.markdown(f"[🔍 Click here to Google this question automatically](https://www.google.com/search?q={q_encoded})")
...
# ==========================================
    ...
    # Gather all questions that were missed, starred, or have notes
    retry_qs = []
    for i, q in enumerate(st.session_state.quiz_data):
        got_wrong = q in st.session_state.incorrect_indices
        has_star = st.session_state.starred.get(i, False)
        has_note = st.session_state.notes.get(i, "").strip() != ""
        
        if got_wrong or has_star or has_note:
            retry_qs.append(q)

    # --- THE NEW RETRY SETTINGS MENU ---
    st.write("---")
    with st.expander("⚙️ Adjust Settings for Next Round", expanded=True):
        st.write("Change these settings before clicking 'Retry' below!")
        r_col1, r_col2 = st.columns(2)
        
        with r_col1:
            retry_shuffle = st.checkbox("Randomize Order", value=True, key="retry_shuffle")
            retry_sim = st.checkbox("DECA Simulation Mode", value=st.session_state.simulation_mode, key="retry_sim")
            retry_back = st.checkbox("Enable 'Previous' Button", value=st.session_state.get('allow_back', True), key="retry_back")
        
        with r_col2:
            retry_timer = st.checkbox("Enable Exam Timer", value=(st.session_state.time_limit > 0), key="retry_timer")
            # Default the timer box to 60 if it was off previously
            prev_time = st.session_state.time_limit if st.session_state.time_limit > 0 else 60
            if retry_timer:
                retry_time_val = st.number_input("Time Limit (Minutes)", min_value=1, max_value=180, value=prev_time, key="retry_time_val")
            else:
                retry_time_val = 0
    ...
    with col2:
        if len(retry_qs) > 0:
            if st.button(f"🔁 Retry {len(retry_qs)} Flagged/Missed"):
                if "history_saved" in st.session_state: del st.session_state.history_saved
                if "sim_scored" in st.session_state: del st.session_state.sim_scored
                
                # Apply the settings selected in the expander above!
                retry_list = retry_qs[:]
                if retry_shuffle: random.shuffle(retry_list)
                
                st.session_state.quiz_data = retry_list
                st.session_state.current_index = 0
                st.session_state.score = 0
                st.session_state.incorrect_indices = []
                st.session_state.user_answers = {}
                ...
                st.session_state.simulation_mode = retry_sim
                st.session_state.allow_back = retry_back
                st.session_state.time_limit = retry_time_val
                st.session_state.start_time = time.time()
            ...
            st.button("🔁 Nothing to Retry!", disabled=True)

    with col3:
        if st.button("🔄 New Exam"):
            # Clicking New Exam just resets and takes you Home so you can pick standard settings
            if "history_saved" in st.session_state: del st.session_state.history_saved
            if "sim_scored" in st.session_state: del st.session_state.sim_scored
            st.session_state.current_index = 0
            st.session_state.score = 0
    ...
    for i, q in enumerate(st.session_state.quiz_data):
        ...
        with st.expander(f"{star_icon} Question {i+1} {status_icon} (Click to expand)"):
            ...
            q_encoded = urllib.parse.quote(q['question_text'])
            st.markdown(f"[🔍 Google this question](https://www.google.com/search?q={q_encoded})")
